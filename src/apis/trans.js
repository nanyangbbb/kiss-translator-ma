...
import { DEEPSEEK_MATH_PROMPT } from "../config/deepseekPrompt";
import { msAuth } from "../libs/auth";
import { fixDupPunct } from "../libs/dupPunct";
...
export const genTransReq = async ({ reqHook, ...args }) => {
  ...
  if (API_SPE_TYPES.ai.has(apiType)) {
    ...
    if (apiType === OPT_TRANS_DEEPSEEK) {
      baseSystemPrompt =
        "你是一个专业的网页翻译引擎。CRITICAL RULE: 文本中的 {数字} 占位符（如 {1}、{2}）是受保护内容，必须原样保留在输出中，绝对禁止修改、删除或翻译这些占位符。严禁在花括号内添加空格（不要写 { 1 }），严禁将花括号替换为其他符号。所有 LaTeX 数学公式已被转换为占位符，翻译完成后会自动还原。你只需专注于翻译普通文本内容。\n\n" +
...
export async function* handleTranslate(
  ...
  if (enableStream) {
    ...
    try {
      ...
      // 4xx（除 429 限流）均为终态错误，标记 noRetry，任务池不再重试
      try {
