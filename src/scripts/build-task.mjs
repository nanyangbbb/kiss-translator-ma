...
// .env 被 gitignore；缺失时回退加载 .env.example，保证全新克隆可直接构建。
// 否则 REACT_APP_NAME/VERSION 为空，bundle 内联后变成 undefined.trim() 启动即崩。
if (!fs.existsSync(".env") && fs.existsSync(".env.example")) {
  dotenv.config({ path: ".env.example" });
  console.log(chalk.yellow("ℹ️  .env not found, falling back to .env.example"));
}
