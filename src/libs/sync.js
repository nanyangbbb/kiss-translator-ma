...
const syncSetting = async () => {
  ...
  // 脱敏后再上传：transApis[].key 里存的是学号（学号即凭证），
  // 同步到 WebDAV/自建 worker 等第三方端点时不应明文外传
  const safe = JSON.parse(JSON.stringify(value));
  (safe?.transApis || []).forEach((a) => {
    if (a && (a.apiType === "DeepSeek" || a.apiSlug === "DeepSeek")) {
      a.key = "";
    }
  });
  const res = await syncData(KV_SETTING_KEY, safe);
  if (res?.isNew) {
